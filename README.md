...
| **3. Native pkg mgr** | `brew` / `apt` / `pacman` / `dnf` | `fish` (login shell), `ghostty`, `gnupg`, `podman`, `podman-compose`, fonts, `coreutils`, `gnu-getopt`, `vault`, `awscli`, `pass` | Things that need real OS integration: login shells, terminal emulators, system crypto, daemons, GUI apps, fonts. |
